...
def mock_track_shipment(tracking_number: str) -> str:
    ...
    if tracking_number in MOCK_TRACKING_DATA:
        ...
        result = f"Shipment {tracking_number} is currently '{data['status']}' in '{data['location']}'."
        logging.info(f"mock_track_shipment result: {result}")
        return result
    else:
        result = f"Tracking number '{tracking_number}' not found."
        logging.info(f"mock_track_shipment result: {result}")
        return result
...
def mock_check_reschedule_availability(tracking_number: str) -> str:
    ...
    if tracking_number in MOCK_RESCHEDULE_ALLOWED:
        if MOCK_RESCHEDULE_ALLOWED[tracking_number]:
            result = "Rescheduling is allowed for this shipment. Please provide the new date (YYYY-MM-DD) and destination postal code."
            logging.info(
                f"mock_check_reschedule_availability result: {result}")
            return result
        else:
            result = "Rescheduling is not allowed for this shipment."
            logging.info(
                f"mock_check_reschedule_availability result: {result}")
            return result
    else:
...
def mock_get_reschedule_dates(tracking_number: str) -> str:
    ...
    if tracking_number in MOCK_RESCHEDULE_DATES and MOCK_RESCHEDULE_ALLOWED.get(tracking_number, False):
        ...
        result = f"Available rescheduling dates for {tracking_number} are: {', '.join(dates)}."
        ...
        result = f"Tracking number '{tracking_number}' not found or no specific dates available."
        logging.info(f"mock_get_reschedule_dates result: {result}")
        return result
    else:  # Not allowed
        result = "Rescheduling is not allowed for this shipment, so no dates can be provided."
        logging.info(f"mock_get_reschedule_dates result: {result}")
        return result
...
def mock_confirm_reschedule(tracking_number: str, new_date: str, postal_code: str) -> str:
    ...
    if tracking_number in MOCK_RESCHEDULE_DATES and new_date in MOCK_RESCHEDULE_DATES.get(tracking_number, []):
        ...
        result = f"Shipment {tracking_number} has been successfully rescheduled to {new_date} for delivery to postal code {postal_code}."
        logging.info(f"mock_confirm_reschedule result: {result}")
        return result
        ...
        result = f"The requested date '{new_date}' is not available or suitable for rescheduling shipment {tracking_number}. Please try get_reschedule_dates to see available options."
        logging.info(f"mock_confirm_reschedule result: {result}")
        return result
...
confirm_reschedule_tool = Tool(
    ...
    description="Use this tool to confirm the rescheduling of a shipment. This requires the tracking number, the new desired date (in YYYY-MM-DD format), and the destination postal code. Ensure the user has provided all three pieces of information and that rescheduling is allowed for the shipment.",
)
...
prompt = ChatPromptTemplate.from_messages(
    [
        ("system",
         ...
         "Your primary goal is to assist users with their shipment queries using the available tools. "
         "When you use a tool, briefly inform the user which tool you are using and what the tool's output signifies before providing your final answer. "
         ...
         "Similarly, if tools like 'confirm_reschedule' require multiple pieces of information (e.g., date, postal code), ensure you have them or ask the user. "
...
while True:
    ...
    try:
        # response = agent_executor.invoke({"query": user_input}, {"return_intermediate_steps": True}) # Example if you need steps
        ...
        # If you uncommented return_intermediate_steps:
        # if "intermediate_steps" in response:
        #     logging.info(f"Intermediate steps: {response['intermediate_steps']}")
